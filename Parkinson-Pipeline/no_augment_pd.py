...
class ExpressionDataset(Dataset):
    def __init__(self, features, labels, lengths, augment=False):
    ...
    def __getitem__(self, idx):
        ...
        return torch.tensor(seq, dtype=torch.float32), self.labels[idx], torch.tensor(self.lengths[idx], dtype=torch.long)
...
def train_model(df=None, feature_names=None):
    ...
    # Generate Report on Validation Set of Best Fold
    print(f"\nGenerating report on Validation Set of Best Fold (Fold {best_fold_idx+1})...")
    
    # Reconstruct the split for the best fold
    target_val_idx = None
    skf = StratifiedKFold(n_splits=NUM_FOLDS, shuffle=True, random_state=42)
    for fold, (train_idx, val_idx) in enumerate(skf.split(X_padded, y_array)):
    ...
    if target_val_idx is not None:
        X_val_best = X_padded[target_val_idx]
        y_val_best = y_array[target_val_idx]
        l_val_best = lengths_array[target_val_idx]
        
        # Validation dataset (NO augmentation)
        val_dataset_best = ExpressionDataset(list(X_val_best), y_val_best, l_val_best, augment=False)
        val_loader_best = DataLoader(val_dataset_best, batch_size=32, shuffle=False, collate_fn=collate_fn)
        ...
        with torch.no_grad():
            for val_X, val_y, val_lengths in val_loader_best:
        ...
        print("\n" + "="*40)
        print(f"Classification Report (Fold {best_fold_idx+1})")
        print("="*40)
        print(report)
        ...
        plt.title(f'Confusion Matrix (Best Model - Fold {best_fold_idx+1})')
        ...
        print("Error: Could not reconstruct validation set for reporting.")
