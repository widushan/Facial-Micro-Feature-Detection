...
for MODEL_TYPE in MODEL_LIST:
    ...
    if final_auc > best_global_auc:
        ...
        best_model_path = os.path.join(OUTPUT_DIR, f"best_{MODEL_TYPE}_model.pth")
        torch.save(model.state_dict(), best_model_path)
        print(f"New best model saved: {best_model_path}")

        # Save the best classification report
        report = classification_report(all_labels, oof_preds_label, target_names=["Healthy","Parkinson"])
        report_path = os.path.join(OUTPUT_DIR, f"best_{MODEL_TYPE}_classification_report.txt")
        with open(report_path, "w") as f:
            f.write(f"Model: {MODEL_TYPE}\n")
            f.write(f"Best OOF AUC: {final_auc:.4f}\n\n")
            f.write(report)
        print(f"Best classification report saved: {report_path}")
