...
OUTPUT_DIR = "Consolidated_Model_Results_v5"
os.makedirs(OUTPUT_DIR, exist_ok=True)

MODEL_LIST = ["efficientnet_b0"]  # Only EfficientNet B0 as per request
...
# ===============================
# 4. Transform & Augmentation
# ===============================
# Techniques to gain more accuracy:
# 1. RandomHorizontalFlip: For micro-expressions, flipping might double the data without loss of semantic meaning.
# 2. RandomAdjustSharpness/ColorJitter: Helps the model be invariant to lighting/noise in spectrograms.
# 3. Normalize with ImageNet stats (already doing).
# 4. SpecAugment (Frequency/Time masking) could be added for signal robustess.
...
def build_model(model_name):
    if model_name == "efficientnet_b0":
        # Using Pretrained Weights is the most standard technique for Small Datasets
        model = models.efficientnet_b0(weights=models.EfficientNet_B0_Weights.DEFAULT)
        
        # Freezing early layers can help if the dataset is very small
        # for param in model.features[:4].parameters():
        #     param.requires_grad = False
            
        num_ftrs = model.classifier[1].in_features
        
        # Enhanced Classification Head: Improved for more complex feature extraction
        head = nn.Sequential(
...
for MODEL_TYPE in MODEL_LIST:
    ...
    for fold, (train_idx, val_idx) in enumerate(sgkf.split(all_paths, all_labels, groups=all_videos)):
        ...
        train_dataset = SpectrogramDataset(train_paths, train_labels, transform_train)
        val_dataset = SpectrogramDataset(val_paths, val_labels, transform_val)
        ...
        # Accuracy Booster: Weight Decay (L2 Regularization) prevents overfitting
        optimizer = optim.AdamW(model.parameters(), lr=LEARNING_RATE, weight_decay=1e-2)
        
        # Accuracy Booster: Learning Rate Scheduler (ReduceLROnPlateau)
