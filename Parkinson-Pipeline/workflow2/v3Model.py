...
NUM_FOLDS = 5
PATIENCE = 6          # Slightly more patient since no augmentation
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
SPECTROGRAM_DIR = "Spectrograms"
OUTPUT_DIR = "model_validation_no_aug"
...
if best_model_state:
    model_save_path = os.path.join(OUTPUT_DIR, f'best_{MODEL_TYPE}_no_aug.pth')
...
plt.title(f'Confusion Matrix ({MODEL_TYPE} - no augmentation)')
plt.tight_layout()
...
disp.plot()
plt.title(f'ROC Curve ({MODEL_TYPE} - no augmentation)')
...
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.title(f'Loss Curves ({MODEL_TYPE} – No Augmentation)')
plt.legend()
...
disp.plot()
plt.title(f'Precision-Recall Curve ({MODEL_TYPE} - no augmentation)')
