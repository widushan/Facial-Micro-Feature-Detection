...
OUTPUT_DIR = "model_validation_trustworthy_new"
...
for fold, (train_idx, test_idx) in enumerate(skf.split(all_paths, all_labels)):
    ...
    fold_accuracies.append(best_fold_acc) # Log the best accuracy for this fold
...
# --------------------------------------------------------------------
# Final evaluation using aggregated OOF predictions (Full Dataset)
# --------------------------------------------------------------------
true = all_true_labels
preds_label = all_preds_label
...
disp.plot()
plt.title(f'Aggregated Precision-Recall Curve ({MODEL_TYPE})')
